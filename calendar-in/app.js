...
/* 要項の保存についての一行。
   ...
   ★ **この道具の利用者はスマホ。** 不具合の手がかりが
     ...
     2026-09-02 に実際に行き詰まったので、画面から読めるようにした。
...
async function uploadOriginal_(file, folderId) {
  ...
  /* このファイルのURLは**主催者以外の人に渡る**。案内文（X・汎用）に載り、
     ...
     ★ **応答を必ず見ること。** `fetch` は HTTP 403 でも例外を投げない。
       2026-08-25〜09-02 はここを try/catch だけで済ませていたため、Googleが共有を
       断っても成功として進み、**主催者には何も出ないままリンクだけが死んでいた**
       （2026-09-02 に実運用で発覚。メンバーがLINEから要項を開けなかった）。
...
/* 案内文は、出欠ができるまで出さない。
   ...
   ★ **この分岐はいま死んでいる。** `ATTEND_AVAILABLE` は無条件に true で、
   出欠は**3言語とも使える**（en/in は案内文の WhatsApp タブにURLが入る）。
   ※ **「en/in に出欠は無い」と読まないこと**。2026-08-26 にこの文で実際に誤った。
    ...
    '<div class="card-foot" style="display:none;margin-top:6px">' +
      ...
      '<div class="ann-wrap">' +
        ...
        /* ★ 渡したのに受付が始まっていないときだけ出す。
           ...
           **「もう作ったつもりの人」には合わない**（2026-09-02 に実運用で発覚）。
  ...
  return fetch(url).then(function (res) {
    if (res.ok) {
      return res.json().then(function (data) {
        ...
        // 表示するモデル名は文言に直書きせず、実際に使う AI_MODELS から採る。
        // 一覧に出ないことがあっても弾かない（キーは有効なのに失敗表示になる方が害が大きい）。
        var model = AI_MODELS[0];
        for (var m = 0; m < AI_MODELS.length; m++) {
          if (names.indexOf(AI_MODELS[m]) >= 0) { model = AI_MODELS[m]; break; }
        }
        return { ok: true, model: model };
      }).catch(function () { return { ok: true, model: AI_MODELS[0] }; });
    }
...
function askAiKey_() {
  ...
  var eyeBtn = document.getElementById('keyEyeBtn');
  var checkEl = document.getElementById('keyCheck');
  ...
  return new Promise(function (resolve) {
    ...
    var setCheck = function (cls, text) {
      if (!checkEl) return;
    ...
    var onEye = function () {
      ...
      eyeBtn.setAttribute('aria-label', I18N.t(toShow ? 'keyModalHide' : 'keyModalShow'));
      input.focus();
    };
    var onSave = function () {
      var k = (input.value || '').trim();
    ...
    if (eyeBtn) {
      ...
      eyeBtn.setAttribute('aria-label', I18N.t('keyModalShow'));
  ...
  if (!link.__wired) {
    ...
    /* 別のタブで団体をつくって戻ってきたら、その場で「管理へ」に変わってほしい。
       ...
       ★ **合鍵をドライブへ預け直すのもここ。** `syncAttendKey_` はもともと
         ...
         （サーバーには SHA-256 しか無い）。2026-09-02 に実運用で発覚した。
