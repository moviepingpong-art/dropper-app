// talk-i18n.js — やりとりドロッパーの多言語辞書（window.I18N）
//
...
(function (global) {
  ...
  var I18N = {
    ja: {
      pageTitle: 'やりとりドロッパー｜会話のスクショから決定事項と未決事項を取り出す',
      appName: 'やりとりドロッパー',
      // ツール切り替えタブ
      typePrompt: 'ドロッパーを選ぶ',
      toolEvent: 'イベント',
      toolSchedule: '予定表',
      toolTalk: 'やりとり',
      toolHint: '入れるものが違います。チラシ1枚ならイベント、予定表ならN件、会話のスクショならやりとり。',
      lead: 'グループのやりとりを読み返して「結局これ決まったんだっけ？」と探す時間をなくします。',
      ...
      priv3: '⚠️ 無料枠では、送信内容がGoogleのサービス改善に利用される場合があります（有料枠では利用されません）。他人が写った会話を入れる前にご確認ください。',
      // ドロップ
      dropTitle: '会話のスクショを入れる',
      dropSub: 'ここにドラッグ、またはタップして選ぶ',
      pickBtn: '画像を選ぶ',
      dropNote: '複数枚をまとめて入れられます（最大10枚）。会話の順に並べてください。上下が重なっていても大丈夫です。',
      filesPicked: '{n}枚を選びました',
      // 基準日
      baseDateLabel: 'この会話の基準日',
      baseDateHint: '「来週の火曜」のような書き方を西暦に直すのに使います。スクショを撮った日を入れてください。',
      runBtn: '読み取る',
      resetBtn: '入れ直す',
      // 状態
      msgQueued: 'AIの順番待ち中…',
      msgRunning: 'AIが会話を読んでいます…',
      msgRetry: 'AIが混雑しています。自動で再試行中…',
      msgDone: '読み取りました。内容を必ずご確認ください。',
      msgNoFiles: '画像が選ばれていません。',
      msgTooMany: '一度に入れられるのは10枚までです。',
      msgTooLarge: '画像の合計が大きすぎます。枚数を減らしてください。',
      ...
      labOwner: '担当',
      labDue: '期限',
      dueFromText: '会話では「{raw}」',
      dueUnknown: '期限の記載なし',
      checkHint: 'AIの読み取りです。特に日付は、右の「会話では」の表現と見比べてご確認ください。',
      copyBtn: 'まとめをコピー',
      copiedBtn: 'コピーしました',
    ...
    en: {
      pageTitle: 'Talk Dropper | Pull decisions and open questions out of chat screenshots',
      appName: 'Talk Dropper',
      typePrompt: 'Choose a dropper',
      toolEvent: 'Event',
      toolSchedule: 'Schedule',
      toolTalk: 'Talk',
      toolHint: 'Each takes something different. One flyer for Event, a schedule sheet for Schedule, chat screenshots for Talk.',
      lead: 'Stop scrolling back through a group chat asking "so what did we actually decide?"',
      privTitle: 'What happens to the images you drop',
      priv1: 'We run no server, so neither the images nor what is read from them reaches us.',
      priv2: 'They go only to Google\'s Gemini API, running on your own key.',
      priv3: '⚠️ On the free tier, what you send may be used to improve Google\'s services (the paid tier does not). Please check this before dropping a conversation that involves other people.',
      dropTitle: 'Drop your chat screenshots',
      dropSub: 'Drag them here, or tap to choose',
      pickBtn: 'Choose images',
      dropNote: 'You can drop several at once (up to 10). Put them in conversation order. Overlapping screenshots are fine.',
      filesPicked: '{n} image(s) selected',
      baseDateLabel: 'Date of this conversation',
      baseDateHint: 'Used to turn wording like "next Tuesday" into a real date. Enter the day the screenshots were taken.',
      runBtn: 'Read it',
      resetBtn: 'Start over',
      msgQueued: 'Waiting in the AI queue…',
      msgRunning: 'The AI is reading the conversation…',
      msgRetry: 'The AI is busy. Retrying automatically…',
      msgDone: 'Done. Please check the result carefully.',
      msgNoFiles: 'No images selected.',
      msgTooMany: 'You can drop at most 10 images at a time.',
      msgTooLarge: 'The images are too large in total. Please use fewer.',
      ...
      labOwner: 'Owner',
      labDue: 'Due',
      dueFromText: 'In the chat: "{raw}"',
      dueUnknown: 'No deadline mentioned',
      checkHint: 'This was read by AI. Check the dates especially, against the original wording shown beside them.',
    ...
    "in": {
      pageTitle: 'Talk Dropper | Chat screenshots se decisions aur open points nikaalein',
      appName: 'Talk Dropper',
      typePrompt: 'Dropper chunein',
      toolEvent: 'Event',
      toolSchedule: 'Schedule',
      toolTalk: 'Talk',
      toolHint: 'Har ek alag cheez leta hai. Ek flyer ke liye Event, schedule sheet ke liye Schedule, chat screenshots ke liye Talk.',
      lead: 'Group chat scroll karke "aakhir tay kya hua?" dhoondhne ka time bachaiye.',
      privTitle: 'Aapke daale gaye images ka kya hota hai',
      ...
      priv3: '⚠️ Free tier par, jo aap bhejte hain wo Google ki services improve karne mein use ho sakta hai (paid tier par nahi). Dusron ki baatcheet daalne se pehle ye dekh lein.',
      dropTitle: 'Chat ke screenshots daalein',
      dropSub: 'Yahan drag karein, ya tap karke chunein',
      pickBtn: 'Images chunein',
      dropNote: 'Ek saath kai daal sakte hain (max 10). Baatcheet ke order mein rakhein. Overlap ho to bhi theek hai.',
      filesPicked: '{n} image chune gaye',
      baseDateLabel: 'Is baatcheet ki tareekh',
      baseDateHint: '"Next Tuesday" jaisi baaton ko asli date banane ke liye. Screenshot lene ka din daalein.',
      runBtn: 'Padhein',
      resetBtn: 'Phir se karein',
      msgQueued: 'AI ki queue mein hain…',
      msgRunning: 'AI baatcheet padh raha hai…',
      msgRetry: 'AI busy hai. Apne aap dobara koshish ho rahi hai…',
      msgDone: 'Ho gaya. Result zaroor check karein.',
      msgNoFiles: 'Koi image nahi chuni gayi.',
      msgTooMany: 'Ek baar mein zyada se zyada 10 images.',
      msgTooLarge: 'Images ka total size bahut bada hai. Kam images use karein.',
      ...
      labOwner: 'Zimmedari',
      labDue: 'Deadline',
      dueFromText: 'Chat mein: "{raw}"',
      dueUnknown: 'Koi deadline nahi likhi thi',
      checkHint: 'Ye AI ne padha hai. Khaas kar dates ko, paas mein diye original shabdon se mila kar dekh lein.',
