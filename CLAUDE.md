...
- （検討中）**モードの選ばせ方を「予定表の形」に変える** — いまは「読み取り方（通常／AI）」を
  選ばせており、利用者に仕組みの理解を求めている。「表形式か、カレンダーのマス目か」を
  図で選ばせ、その結果としてモードが決まる形にしたい。
  ※マス目をOCRで読むと、0件にならず**それらしい件数のまま日付と行事名の対応だけが狂う**。
  自動判定は危険なので、選ぶのは利用者。
