...
- **都道府県だけの行から後ろは会場**（`PREF_RE`）— 「都道府県」列と「会場」列が分かれた表
  （Tリーグ日程表など）では、日付のあとに 対戦・時刻・都道府県・会場 が1行ずつ続く。
  これを分けないと会場名が行事名に混ざり、**カレンダーの「場所」が空**になって地図が出ない。
  **47都道府県の完全一致に限ること。**「◯◯県」で広く拾うと「県大会」を場所と読み違える。
  都道府県が先頭に来たときは行事名が消えるので分けない。都道府県は場所側に含める（地図の精度）。
- **表が2つある予定表** — 2つ目の見出しが1つ目の最後の予定に吸い込まれる
  （`彩たま - 金沢 埼玉県 浦和駒場体育館 試合日 対戦` になった）。実物で見つけた見出し語を
  `HEADER_RE` に足していくこと。
