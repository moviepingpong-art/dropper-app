...
(function (global) {
  ...
  var I18N = {
    ja: {
      ...
      // ★ ほかの3本（decide/schedule）と同じ中身。直すときは4本ぶん直すこと
      // --- 「何ができる？」ポップアップ（#tools-modal）---
      tmBtn: '🔍 何ができる？',
      ...
      pfNeed: '必須',
      pfAny: '任意',
      pfLead: '申込書に反映させる項目を入力してください。',
      pfAnyNote: '入れた項目が申込書に書き込まれます。必須は姓と名だけで、分からない項目は空のままでも保存できます（空の欄は申込書では飛ばします）。',
      formOkBtn: 'この人を入れる',
      formCancel: 'やめる',
      ...
      // ★ 様式が無いときに作る（2026-09-26）。「大会」とは限らないので、題はこちらで作らない
      formMake: '様式がないとき：項目を選んで作る',
      makeTitleLabel: '申込書の題',
      makeTitlePh: '例：第10回 ○○大会 参加申込書／○○教室 受講申込書',
      makeTitleHint: '1行目にそのまま書きます。空のままなら「申込書」とだけ書きます。',
      makeItemsLabel: '名簿から書く項目',
      makeName: '氏名（いつも入ります）',
      makeExtrasLabel: '名簿に足した項目',
      makeExtrasNoRoster: '①で名簿を開くと、名簿に足した項目も選べます。',
      makeOthersLabel: '名簿にない項目（種目・備考など）',
      makeOthersPh: '項目の名前',
      makeOthersAdd: '足す',
      makeOthersHint: '見出しだけの空いた列を作ります。中身は、保存した申込書に手で書いてください。',
      makeOthersDup: '「{name}」はもう入っています。',
      makeBaseLabel: '年齢の基準日（年齢を書くとき）',
      makeBaseHint: '入れると、申込書の2行目に「※年齢は ○年○月○日 現在」と書き、この日の年齢を書きます。空のままなら、④で聞きます。',
      makeRowsLabel: '記入行の数',
      makeRowsHint: '1枚に{max}行まで作れます。人数がそれより多いときは、③で2枚目を足して続きを書けます。',
      makeGo: '作って次へ',
      makeCancel: 'やめる',
      makeMaking: '申込書を作っています…',
      makeDone: '「{name}」を作りました（{n}行）。③で入れる人を選んでください。',
      makeNeedRoster: '「{name}」を作りました（{n}行）。①で名簿を開くと、③へ進みます。',
      ...
      teachHint: '名前を書く列と行を教えてください。下の見取り図で、最初の人の名前を書くマスと、最後の人のマスを押すと入ります（列の英字と行の番号を打ち込むこともできます）。',
      // --- シートを1枚ずつ（2026-09-24） ---
      shChip: 'シート{i} {name}',
      shChipCount: '（{n}人）',
      shPrev: '← 前のシート',
      shNext: 'このシートを終えて、次のシート（{name}）へ →',
      shMoved: '「{name}」の表がぜんぶそろったので、次のシートに移りました。',
      shNextHint: 'このシートが終わったら、下の「次のシートへ」を押してください（シート{i}／{n}）。',
      pickMoreRoom: '{n}人を入れました。{table} にまだ入れられます（全部うめなくても次へ進めます）。',
      // --- 表の名前と「いま入れている表」（2026-09-24）。申込書には書かない、見せるだけ ---
      tblNow: 'いま入れている表：',
      tblCount: '{n}人／{cap}人',
      tblAllDone: 'すべての表がそろいました。',
      tblNamed: '表{n}　{name}',
      tblHeads: '表{n}（{heads}）',
      tblPlain: '表{n}',
      tblNameLabel: 'この表：',
      tblNamePh: '名前を付けられます（例：男子シングルス、ソプラノ、小学生の部）',
      tblFromForm: '申込書の文字から',
      // --- 申込書の見取り図（2026-09-24） ---
      svPickHint: '申込書の見取り図です。色の付いた欄に、選んだ順に名前が入ります。',
      svCut: '長い様式なので、{from}〜{to} のあたりだけを描いています。',
      svTeachStart: '最初の人の名前を書くマスを押してください。',
      svTeachEnd: '{col}列 {from}行目から。最後の人の名前を書くマスを押してください。',
      svTeachDone: '{col}列の {from}〜{to}行目を選びました。よければ下の「この形で読む」を押してください（マスを押すと選び直せます）。',
      teachCol: '名前を書く列',
      teachRows: '書く行',
      teachApply: 'この形で読む',
